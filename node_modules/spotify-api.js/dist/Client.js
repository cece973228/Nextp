...
class Client {
    ...
    async search(query, options) {
        ...
        if (fetchedData.albums)
            response.albums = Cache_1.createCacheStructArray('albums', this, fetchedData.albums.items);
        if (fetchedData.tracks)
            response.tracks = Cache_1.createCacheStructArray('tracks', this, fetchedData.tracks.items);
        if (fetchedData.episodes)
            response.episodes = Cache_1.createCacheStructArray('episodes', this, fetchedData.episodes.items);
        if (fetchedData.shows)
            response.shows = Cache_1.createCacheStructArray('shows', this, fetchedData.shows.items);
        if (fetchedData.artists)
            response.artists = Cache_1.createCacheStructArray('artists', this, fetchedData.artists.items);
        return response;
    }
    /**
