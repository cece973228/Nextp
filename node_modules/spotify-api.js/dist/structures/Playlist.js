...
class Playlist {
    ...
    constructor(data, client) {
        ...
        this.name = data.name;
        this.owner = Cache_1.createCacheStruct('users', client, data.owner);
...
function createPlaylistTracks(client, rawPlaylistTracks) {
    const createTrack = client.cacheSettings.tracks && client.cacheSettings.episodes
        ? (track) => Cache_1.createForcedCacheStruct(`${track.type}s`, client, track)
        : (track) => track.type == "track"
            ? new Track_1.Track(track, client)
            : new Episode_1.Episode(track, client);
    return rawPlaylistTracks.map(x => ({
        addedAt: x.added_at,
        addedBy: Cache_1.createCacheStruct('users', client, x.added_by),
        isLocal: x.is_local,
