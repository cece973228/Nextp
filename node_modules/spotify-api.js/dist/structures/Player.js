...
function createCurrentlyPlaying(client, data) {
    return {
        ...
        item: data.item ? Cache_1.createCacheStruct(`${data.item.type}s`, client, data.item) : null,
        context: {
...
function createRecentlyPlayed(client, data) {
    return {
        items: data.items.map(x => ({ track: Cache_1.createCacheStruct('tracks', client, x.track), playedAt: x.played_at })),
